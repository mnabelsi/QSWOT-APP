...
For each field:
  - key: camelCase identifier (e.g. "monthlyVisitors", "hospitalBeds", "annualBudget")
  - name: human-readable label (e.g. "Monthly Visitors", "Hospital Beds", "Annual Budget")
  - type: "number" | "text" | "select"
  - unit: (number type only) short unit label, e.g. "beds", "K visitors/mo", "K€"
  - options: (select type only) array of 3–6 string values

Return ONLY a JSON array (nothing before or after):
[{"key":"hospitalBeds","name":"Hospital Beds","type":"number","unit":"beds"}]`,

  marketResearch: `List 8–10 top potential client accounts for {company_name} ({industry}) in: {territory}.

Focus on accounts that represent high business value for this industry.

For each account provide:
  - name: realistic and specific institution/company name
  - size: estimated annual revenue potential in K€ (integer)
  - type: one of → hospital | clinic | surgical_center | university_hospital | distributor | gpo | other
  - beds: bed count as integer (null if not applicable)
  ...
  - notes: one sentence why this is a strong target
  - customFields: object with estimated values for these industry-specific fields: {custom_fields_spec}

Return ONLY a JSON array (nothing before or after):
[{"name":"...","size":500,"type":"hospital","beds":450,"territory":"...","ownership":"public","contractStatus":"prospect","strategicPriority":"high","zone":"green","notes":"...","customFields":{}}]`,
} as const;
